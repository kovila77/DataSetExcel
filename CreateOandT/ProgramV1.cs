...
namespace CreateOandT
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int COUNT_OF_D = 1;
            int COUNT_OF_NOISE = 10;
            double LEFT_NOISE = -0.001;
            double RIGHT_NOISE = 0.001;
            ...
            string fName = ofd.FileName; // Файл Excel, с которым производится работа
            //string fName = @"C:\Users\35498\source\repos\DataSetExcel\Neuro\NewData\DataT.xlsx"; // Файл Excel, с которым производится работа
            ...
            try
            {
                ...
                var rnd = new Random();
                for (int i = 0; i < (int)(countOfRow / 11); i++)
                {
                ...
                foreach (int rowIndex in teachSet)
                {
                    for (int j = 1; j <= counOfColumn; j++)
                    {
                        ...
                        double val = (double)sheet.Cells[rowIndex, j];
                        teachSheet.Cells[rowToIndex, j] = val;
                        for (int k = 0; k < COUNT_OF_NOISE; k++)
                        {
                            rowToIndex++;
                            teachSheet.Cells[rowToIndex, j] = val * ((double)rnd.Next((int)(LEFT_NOISE * 1000000), (int)(RIGHT_NOISE * 1000000)) / 1000000.0);
                        }
                    }
                    rowToIndex++;
                }
                ...
                foreach (int i in testSet)
                {
                    for (int j = 1; j <= counOfColumn; j++)
                    {
                        double val = (double)sheet.Cells[i, j];
                        testSheet.Cells[rowToIndex, j] = val;
                        for (int k = 0; k < COUNT_OF_NOISE; k++)
                        {
                            rowToIndex++;
                            testSheet.Cells[rowToIndex, j] = val * ((double)rnd.Next((int)(LEFT_NOISE * 1000000), (int)(RIGHT_NOISE * 1000000)) / 1000000.0);
                        }
                    }
                    rowToIndex++;
                }
